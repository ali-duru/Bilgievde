...
export default function LoadMore({ posts, setPosts, taxonomy = null }) {
	let bT = posts.pageInfo.hasNextPage
		...
		: 'Bütün yazılarımıza erişim sağladınız';
	...
	const handleOnclick = async (event) => {
		...
		if (morePosts.pageInfo.hasNextPage) {
			setButtonText('Daha Fazlası için tıklayınız');
			setButtonDisabled(false);
		} else {
			setButtonText('Bütün yazılarımıza erişim sağladınız');
			setButtonDisabled(true);
		}
	};
	return (
		<button
			className="load-more font-bold bg-blue-500 text-white px-4 py-2 hover:bg-blue-600"
