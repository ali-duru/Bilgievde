...
export async function getPostList(endCursor = null, taxonomy = null) {
	let condition = `after: "${endCursor}", first: 8, where: {orderby: {field: DATE, order: DESC}}`;

	if (taxonomy) {
		condition = `after: "${endCursor}", first: 8, where: {orderby: {field: DATE, order: DESC}, ${taxonomy.key}: "${taxonomy.value}"}`;
	}
